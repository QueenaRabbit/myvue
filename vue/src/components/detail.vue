<template>
  <div class="detail">
    <div class="nav">
      <ul>
        <!--<li class="l-btn"></li>-->
        <router-link tag="li" class="l-btn" to="/home"></router-link>
      </ul>
    </div>
    <div class="content">
      {{$route.params.id}}|{{$route.query}}|{{$route.path}}
      <div class="header clear"><h2><img :src="article.author_face" alt="" width="100"/></h2><p>作者名字</p></div>
      <div class="cont">
        <h3>{{article.title}}</h3>
        <div class="time"><p>{{article.time | date}}<span><img src="src/assets/img/zan.png" alt=""/></span></p></div>
...
<script>
  export default{
    ...
    mounted(){
      ...
      //兜库
      this.$http({
//        url:'http://localhost:80/article.php',
